...
def load_gitlab_repository_metadata(repository_url, header, readme_only=False):
    ...
    #
    # if len(notebooks) > 0:
    #     filtered_resp["hasExecutableNotebook"] = [convert_to_raw_user_content_gitlab(x, owner, repo_name, repo_ref) for
    #                                               x
    #                                               in
    #                                               notebooks]
    # if len(dockerfiles) > 0:
    #     filtered_resp["hasBuildFile"] = [convert_to_raw_user_content_gitlab(x, owner, repo_name, repo_ref) for x in
    #                                      dockerfiles]
    # if len(docs) > 0:
    #     filtered_resp["hasDocumentation"] = docs
    #
    # if len(scriptfiles) > 0:
    #     filtered_resp["hasScriptFile"] = [convert_to_raw_user_content_gitlab(x, owner, repo_name, repo_ref) for x in
    #                                       scriptfiles]
...
def load_github_repository_metadata(repository_url, header, ignore_github_metadata=False, readme_only=False):
    ...
    #             if filename.endswith(".sh"):
    #                 # scriptfiles.append(os.path.join(repo_relative_path, filename))
    #                 script_files.append(repo_relative_path + "/" + filename)
    #             if filename.endswith(".ttl") or filename.endswith(".owl") or filename.endswith(".nt") or filename. \
    #                     endswith(".xml"):
    #                 uri = extract_ontologies.is_file_ontology(filename)
    #                 # Add onto
    #                 if uri is not None:
    ...
    #                     onto = {
    #                         "uri": uri,
    #                         "file_url": repo_relative_path + "/" + filename
    #                     }
    ...
    #
    # if len(notebooks) > 0:
    #     filtered_resp["hasExecutableNotebook"] = [convert_to_raw_user_content_github(x, owner, repo_name, repo_ref) for
    #                                               x in
    #                                               notebooks]
    # if len(dockerfiles) > 0:
    #     filtered_resp["hasBuildFile"] = [convert_to_raw_user_content_github(x, owner, repo_name, repo_ref) for x in
    #                                      dockerfiles]
    # if len(docs) > 0:
    #     filtered_resp["hasDocumentation"] = docs
    #
    # if len(script_files) > 0:
    #     filtered_resp["hasScriptFile"] = [convert_to_raw_user_content_github(x, owner, repo_name, repo_ref) for x in
    #                                       script_files]
    # if len(ontologies) > 0:
    #     for onto in ontologies:
    #         onto["file_url"] = convert_to_raw_user_content_github(onto["file_url"], owner, repo_name, repo_ref)
...
def load_local_repository_metadata(local_repo):
    ...
    #         if filename.endswith(".sh"):
    #             script_files.append(os.path.join(repo_dir, repo_relative_path, filename))
    #
    #     for dirname in dirnames:
    #         if dirname.lower() == "docs":
    #             if repo_relative_path == ".":
