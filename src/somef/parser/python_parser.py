import ast
import tomli
import logging
import re
from pathlib import Path
from ..process_results import Result
from ..utils import constants

"""
This code is inpisred by Codemeta Project parsers, specifically python.py
"""

def parse_dependency(dependency_str):
    if not dependency_str:
        return None, None
        
    parts = re.split(r'(>=|<=|==|!=|>|<|~=|\[)', dependency_str, 1)
    name = parts[0].strip()
    if len(parts) > 1:
        version = ''.join(parts[1:])
    else:
        version = ""

    version = version.strip("[]() -.,:")
    
    return name, version

def parse_url_type(url_label):
    label = url_label.lower()
    
    if "repository" in label or label in ("git", "github", "code", "sourcecode", "source"):
        return constants.CAT_CODE_REPOSITORY
    elif "issue" in label or label in ("bug tracker", "tracker", "issues"):
        return constants.CAT_ISSUE_TRACKER
    elif "doc" in label or label in ("documentation", "api reference", "reference"):
        return constants.CAT_DOCUMENTATION
    elif label == "readme":
        return constants.CAT_README_URL
    elif "download" in label:
        return constants.CAT_DOWNLOAD_URL
    else:
        return constants.CAT_RELATED_DOCUMENTATION

def parse_author_string(author_str):
    if not author_str:
        return {"name": None, "email": None}
    
    email_match = re.search(r'<([^>]+)>', author_str)
    if email_match:
        email = email_match.group(1)
        name = author_str[:email_match.start()].strip()
        return {"name": name, "email": email}
    
    return {"name": author_str, "email": None}

def parse_python_metadata(file_path, metadata_result: Result):
    try:
        if Path(file_path).name == "pyproject.toml":
            with open(file_path, "rb") as f:
                data = tomli.load(f)
                
                project = data.get("project", {})
                
                if not project and "tool" in data and "poetry" in data["tool"]:
                    project = data["tool"]["poetry"]
                
                if "name" in project:
                    metadata_result.add_result(
                        constants.CAT_NAME,
                        {
                            "value": project["name"], 
                            "type": constants.STRING
                        },
                        1,
                        constants.TECHNIQUE_CODE_CONFIG_PARSER,
                        file_path
                    )
                
                if "description" in project:
                    metadata_result.add_result(
                        constants.CAT_DESCRIPTION,
                        {
                            "value": project["description"], 
                            "type": constants.STRING
                        },
                        1,
                        constants.TECHNIQUE_CODE_CONFIG_PARSER,
                        file_path
                    )
                
                if "version" in project:
                    metadata_result.add_result(
                        constants.CAT_RELEASES,
                        {
                            "value": project["version"],
                            "type": constants.RELEASE,
                            "tag": project["version"]
                        },
                        1,
                        constants.TECHNIQUE_CODE_CONFIG_PARSER,
                        file_path
                    )
                
                dependencies = project.get("dependencies", [])
                if isinstance(dependencies, list):
                    for req in dependencies:
                        name, version = parse_dependency(req)
                        if name:
                            metadata_result.add_result(
                                constants.CAT_REQUIREMENTS,
                                {
                                    "value": req, 
                                    "version": version, 
                                    "type": "dependency"
                                },
                                1,
                                constants.TECHNIQUE_CODE_CONFIG_PARSER,
                                file_path
                            )
                elif isinstance(dependencies, dict):
                    for name, version in dependencies.items():
                        req = f"{name}{version}" if version else name
                        metadata_result.add_result(
                            constants.CAT_REQUIREMENTS,
                            {
                                "value": req, 
                                "name": name, 
                                "version": version, 
                                "type": "dependency"
                            },
                            1,
                            constants.TECHNIQUE_CODE_CONFIG_PARSER,
                            file_path
                        )
                
                
                urls = project.get("urls", {})
                if not urls and "tool" in data and "poetry" in data["tool"]:
                    
                    poetry = data["tool"]["poetry"]
                    if "repository" in poetry:
                        urls["repository"] = poetry["repository"]
                    if "homepage" in poetry:
                        urls["homepage"] = poetry["homepage"]
                    if "documentation" in poetry:
                        urls["documentation"] = poetry["documentation"]
                
                for url_type, url in urls.items():
                    category = parse_url_type(url_type)
                    metadata_result.add_result(
                        category,
                        {
                            "value": url, 
                            "type": constants.URL
                        },
                        1,
                        constants.TECHNIQUE_CODE_CONFIG_PARSER,
                        file_path
                    )
                
                authors = project.get("authors", [])
                if isinstance(authors, list):
                    for author in authors:
                        if isinstance(author, dict):
                            author_data = {
                                "name": author.get("name"),
                                "email": author.get("email"),
                                "url": author.get("url"),
                                "type": "author",
                                "value": author.get("name")
                            }
                        else:
                
                            parsed = parse_author_string(author)
                            author_data = {
                                "name": parsed["name"],
                                "email": parsed["email"],
                                "type": "author",
                                "value": parsed["name"]
                            }
                        
                        metadata_result.add_result(
                            constants.CAT_AUTHORS,
                            author_data,
                            1,
                            constants.TECHNIQUE_CODE_CONFIG_PARSER,
                            file_path
                        )

                if "license" in project:
                    license_info = project["license"]
                    if isinstance(license_info, dict):
                        license_text = license_info.get("text", "")
                        license_value = license_info.get("type", "")
                    else:
                        license_text = ""
                        license_value = license_info
                        
                    metadata_result.add_result(
                        constants.CAT_LICENSE,
                        {
                            "value": license_value,
                            "spdx_id": license_value,
                            "type": constants.LICENSE
                        },
                        1,
                        constants.TECHNIQUE_CODE_CONFIG_PARSER,
                        file_path
                    )
                
                if "keywords" in project:
                    for keyword in project["keywords"]:
                        metadata_result.add_result(
                            constants.CAT_KEYWORDS,
                            {
                                "value": keyword, 
                                "type": constants.STRING
                            },
                            1,
                            constants.TECHNIQUE_CODE_CONFIG_PARSER,
                            file_path
                        )
                
            metadata_result.add_result(
                constants.CAT_PACKAGE_FILE,
                {
                    "value": "pyproject.toml",
                    "type": "python",
                },
                1,
                constants.TECHNIQUE_CODE_CONFIG_PARSER,
                file_path
            )

################################ setup.py #########################################
        
        elif Path(file_path).name == "setup.py":
            with open(file_path, "r", encoding="utf-8") as f:
                try:
                    tree = ast.parse(f.read())
                    
                    for node in ast.walk(tree):
                        if isinstance(node, ast.Call) and getattr(node.func, "id", "") == "setup":
                            for keyword in node.keywords:
                                
                                if keyword.arg == "name":
                                    try:
                                        name = ast.literal_eval(keyword.value)
                                        metadata_result.add_result(
                                            constants.CAT_NAME,
                                            {"value": name, "type": constants.STRING},
                                            1,
                                            constants.TECHNIQUE_CODE_CONFIG_PARSER,
                                            file_path
                                        )

                                    except (ValueError, SyntaxError):
                                        pass
                                        
                                elif keyword.arg == "version":
                                    try:
                                        version = ast.literal_eval(keyword.value)
                                        metadata_result.add_result(
                                            constants.CAT_RELEASES,
                                            {
                                                "value": version,
                                                "type": constants.RELEASE,
                                                "tag": version
                                            },
                                            1,
                                            constants.TECHNIQUE_CODE_CONFIG_PARSER,
                                            file_path
                                        )
                                    except (ValueError, SyntaxError):
                                        pass
                                        
                                elif keyword.arg == "install_requires":
                                    try:
                                        requirements = ast.literal_eval(keyword.value)
                                        if isinstance(requirements, list):
                                            for req in requirements:
                                                name, version = parse_dependency(req)
                                                metadata_result.add_result(
                                                    constants.CAT_REQUIREMENTS,
                                                    {
                                                        "value": req, 
                                                        "name": name, 
                                                        "version": version, 
                                                        "type": "dependency"},
                                                    1,
                                                    constants.TECHNIQUE_CODE_CONFIG_PARSER,
                                                    file_path
                                                )
                                    except (ValueError, SyntaxError):
                                        pass
                                        
                                elif keyword.arg == "author":
                                    try:
                                        author = ast.literal_eval(keyword.value)
                                        metadata_result.add_result(
                                            constants.CAT_AUTHORS,
                                            {
                                                "value": author,
                                                "email": " ",
                                                "type": "author"
                                            },
                                            1,
                                            constants.TECHNIQUE_CODE_CONFIG_PARSER,
                                            file_path
                                        )
                                    except (ValueError, SyntaxError):
                                        pass
                                        
                                elif keyword.arg == "email":
                                    try:
                                        email = ast.literal_eval(keyword.value)
                                        for result in metadata_result.results.get(constants.CAT_AUTHORS, []):
                                            result["result"]["email"] = email
                                    except (ValueError, SyntaxError):
                                        pass
                                        
                                elif keyword.arg == "description":
                                    try:
                                        description = ast.literal_eval(keyword.value)
                                        metadata_result.add_result(
                                            constants.CAT_DESCRIPTION,
                                            {
                                                "value": description, 
                                                "type": constants.STRING
                                            },
                                            1,
                                            constants.TECHNIQUE_CODE_CONFIG_PARSER,
                                            file_path
                                        )
                                    except (ValueError, SyntaxError):
                                        pass
                                        
                                elif keyword.arg == "url":
                                    try:
                                        url = ast.literal_eval(keyword.value)
                                        metadata_result.add_result(
                                            constants.CAT_CODE_REPOSITORY,
                                            {
                                                "value": url, 
                                                "type": constants.URL
                                                },
                                            1,
                                            constants.TECHNIQUE_CODE_CONFIG_PARSER,
                                            file_path
                                        )
                                    except (ValueError, SyntaxError):
                                        pass
                                        
                                elif keyword.arg == "license":
                                    try:
                                        license_value = ast.literal_eval(keyword.value)
                                        metadata_result.add_result(
                                            constants.CAT_LICENSE,
                                            {
                                                "value": license_value,
                                                "type": constants.LICENSE
                                            },
                                            1,
                                            constants.TECHNIQUE_CODE_CONFIG_PARSER,
                                            file_path
                                        )
                                    except (ValueError, SyntaxError):
                                        pass
                                        
                except SyntaxError:
                    logging.warning(f"Syntax error in {file_path}, couldn't parse setup.py")

            metadata_result.add_result(
                constants.CAT_PACKAGE_FILE,
                {
                    "value": "setup.py",
                    "type": "python",
                },
                1,
                constants.TECHNIQUE_CODE_CONFIG_PARSER,
                file_path
            )
                
    except Exception as e:
        logging.error(f"Error parsing Python metadata from {file_path}: {str(e)}")
    
    return metadata_result